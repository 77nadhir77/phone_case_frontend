...
export default function Upload() {
  ...
  return (
    ...
    <MaxWidthWrapper className='flex flex-1 flex-col mt-2 mb-2 items-center'>
      ...
      <Steps/>
      <div className={`flex flex-grow flex-1 ${isDragOver ? 'bg-blue-900/10' : 'bg-gray-900/5'} items-center justify-center w-2/3 p-2 rounded-xl`}>
        <Dropzone
          ...
          >
          {
            ({getRootProps, getInputProps}) => (
              <div className="relative w-full max-w-lg flex h-full flex-col items-center justify-center" {...getRootProps()}>
                  <input {...getInputProps()}/>
                  ...
                  <div className='flex flex-col mb-2 justify-center text-sm text-zinc-700'>
                    {
                      isUploading ? (
                        <div className='flex flex-col items-center'>
                          <p>Uploading...</p>
                          <progress className='mt-2 w-40 h-2 bg-gray-300' value={uploadProgress} max={100} />
                        </div>
                      )
                      :isPending ? (
                        <div className='flex flex-col items-center'>
                    ...
                    {
                      isPending? null : (<p className='text-xs text-zinc-500'>JPEG, JPG, PNG</p>)
                    }
                  </div>
                </div>
              )
